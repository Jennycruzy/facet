...
| Browser launcher wallet boundary | **Staged, 26 August 2026** | `packages/web/launch.html` and `assets/js/wallet-binding.js` connect an injected EIP-1193 EOA and request one origin/network/pool-bound `personal_sign` message. The signature stays in memory; identity derivation, proving, and broadcast remain disabled. |
